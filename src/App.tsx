          ...
          {isModalOpen && <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div className="bg-white pt-7 pb-10 px-5 rounded w-96">
              ...
              <div className="flex justify-between grow-1 mb-5">
                ...
                <button className={`${activeStep === 2 ? "bg-rose-900": "bg-gray-400"} bg-gray-400 px-2 py-3 rounded-r basis-1/3`}>Step 3</button>
              </div>
              ...
              {/*STEP 1*/}
                {activeStep === 0 && <>
                    ...
                    <form onSubmit={onSubmitHandler} className="flex flex-col">
                        <input value={userEmail} onChange={({target:{value}}) => setUserEmail(value)} className="p-2 mb-5 w-12/12 bg-gray-200 border-b-2 border-slate-700 rounded-t-sm" type="email" name="email" placeholder="your email" />
                        <select value={productType} onChange={({target:{value}}) => setProductType(value)} name="product" className="p-2 mb-5 bg-gray-200 border-b-2 border-slate-700 rounded-t-sm">
                            <option value="electronics">Electronics</option>
                            <option value="clothes">Clothes</option>
                        </select>
                        <input value={productQuantity} onChange={({target:{value}}) => setProductQuantity(Number(value))} name="quantity" placeholder="quantity" className="p-2 mb-5 bg-gray-200 border-b-2 border-slate-700 rounded-t-sm"/>
              ...
              {/*STEP 2 */}
                {activeStep === 1 && <>
                    ...
                    <button onClick={() => setActiveStep(2)} className="p-2 rounded-md bg-slate-700 text-white">Go to summary</button>
                </>}
              ...
              {/*STEP 3*/}
                {activeStep === 2 && <>
                    ...
                    <button className="p-2 rounded-md bg-slate-700 text-white">Go to summary</button>
                </>}
            </div>
          </div>}
        </div>
